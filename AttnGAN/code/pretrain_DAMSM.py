...
from MRI_dataset import prepare_mri_data, get_mri
...
def evaluate(dataloader, cnn_model, rnn_model, batch_size):
    ...
    for step, data in enumerate(dataloader, 0):
        if cfg.DATASET_NAME == "mri":
            real_imgs, captions, cap_lens, class_ids, keys = prepare_mri_data(data)
        else:
...
if __name__ == "__main__":
    ...
    if cfg.DATASET_NAME == "mri":
        dataset, dataset_val, _ = get_mri(BATCH_SIZE=cfg.TRAIN.BATCH_SIZE)
        dataset.n_words = 1
        dataset.ixtoword = None
