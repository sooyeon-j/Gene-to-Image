...
from MRI_dataset import prepare_mri_data
...
class condGANTrainer(object):
    def __init__(self, output_dir, data_loader, n_words, ixtoword):
        ...
        # torch.cuda.set_device("cuda:{}".format(cfg.GPU_ID))
        ...
        # self.fid = FIDMetric()
        self.ms_ssim = MultiScaleSSIMMetric(
            spatial_dims=2, data_range=1.0, kernel_size=4
        )
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.max_epoch):
            ...
            while step < self.num_batches:
                ...
                data = next(data_iter)
                # data = data_iter.next()
                if cfg.DATASET_NAME != "mri":
                    imgs, captions, cap_lens, class_ids, keys = prepare_data(data)
                else:
                    imgs, captions, cap_lens, class_ids, keys = prepare_mri_data(data)
    ...
    def sampling(self, split_dir):
        ...
        if cfg.TRAIN.NET_G == "":
            ...
            pfw.set_config(batch_size=self.batch_size)
            ...
            for e in range(EPOCHS):  # (cfg.TEXT.CAPTIONS_PER_IMAGE):
                synth_features = []
                real_features = []
                ...
                for step, data in enumerate(self.data_loader, 0):
                    ...
                    if cfg.DATASET_NAME != "mri":
                        ...
                        imgs, captions, cap_lens, class_ids, keys = prepare_mri_data(
                            data
                        )
                    ...
                    # Metrics
                    #############
                    decoded = (fake_imgs[2] + 1) / 2
                    images = (imgs[2].type(torch.float32) + 1) / 2
                    n = len(images)
                    vutils.save_image(decoded, "temp_before.png")
                    ...
                    for idx_a, idx_b in idx_pairs:
                        ms_ssim_scores.append(
                            self.ms_ssim(decoded[[idx_a]], decoded[[idx_b]])
                        )
                        ssim_scores.append(
                            self.ssim(decoded[[idx_a]], decoded[[idx_b]])
                        )

                    ####
                    vutils.save_image(decoded, "temp.png")
                    for j in range(batch_size):
                        s_tmp = "%s/single/%s/%s" % (save_dir, class_ids[j], keys[j])
                        folder = s_tmp[: s_tmp.rfind("/")]
                        if not os.path.isdir(folder):
                            print("Make a new folder: ", folder)
                            mkdir_p(folder)
                        ...
                        fullpath = "%s_s%d.png" % (s_tmp, cnt)
                        cnt += 1
                        im = decoded[j].detach()
                        vutils.save_image(im, fullpath)
                        # im.save(fullpath)

                psnr_scores = torch.cat(psnr_scores, dim=0)
                print({"PSNR Scores": psnr_scores.mean()})

                # synth_features = torch.vstack(synth_features)
                # real_features = torch.vstack(real_features)

                # fid_res = self.fid(synth_features, real_features)
                fid_res = pfw.fid(decoded, images)

                print({"FID Scores": fid_res.item()})
            ...
            print(f"FID Scores :{np.mean(fid_total):.3f}")
