...
class RNN_ENCODER(nn.Module):
    ...
    def define_module(self):
        if cfg.DATASET_NAME == "mri":
            self.encoder = nn.Linear(1, self.ninput)
        else:
        ...
        if self.rnn_type == "LSTM":
            ...
            temp = []
            temp.append(nn.Linear(self.ninput, self.nhidden))
            for i in range(self.nlayers - 1):
                temp.append(nn.Linear(self.nhidden, self.nhidden))
            self.rnn = nn.Sequential(*temp)
    ...
    def forward(self, captions, cap_lens, hidden, mask=None):
        ...
        if cfg.DATASET_NAME == "mri":
            emb = self.drop(self.encoder(captions.unsqueeze(-1)))  # B, l, 300
        else:
            emb = self.drop(self.encoder(captions))  # B, l, 300
...
class D_GET_LOGITS(nn.Module):
    ...
    def forward(self, h_code, c_code=None):
        # h_code: [20, 512, 4, 4] c_code: [20, 256]
