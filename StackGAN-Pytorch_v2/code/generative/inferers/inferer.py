...
class DiffusionInferer(Inferer):
    ...
    def __call__(
        ...
        if mode == "concat":
            ...
            condition = None
        prediction = diffusion_model(x=noisy_image, timesteps=timesteps, context=condition, class_labels = labels, clinical_data = clinic)
    ...
    def sample(
        ...
        if guidance_scale > 0:
            if conditioning is not None and labels is not None:
                uncond_labels = 3 * torch.ones_like(labels)
                uncond_conditioning = -1 * torch.ones_like(conditioning)
            elif conditioning is not None and labels is None:
                uncond_labels = labels
                uncond_conditioning = 3 * torch.ones_like(conditioning)
            elif conditioning is None and labels is not None:
                uncond_labels = 3 * torch.ones_like(labels)
                uncond_conditioning = conditioning
            
        for t in progress_bar:
            timesteps = (torch.ones(image.shape[0]) * t).long().to(image.device)
            # 1. predict noise model_output
            if mode == "concat":
                model_input = torch.cat([image, conditioning], dim=1)
                model_output = diffusion_model(
                    model_input, timesteps=timesteps, class_labels=labels
                )
                if guidance_scale > 0:
                    uncond_output = diffusion_model(model_input, timesteps=timesteps, class_labels=uncond_labels, clinical_data = clinic)
                    model_output = torch.lerp(uncond_output, model_output, guidance_scale)
            else:
                model_output = diffusion_model(
                    image, timesteps=timesteps, context=conditioning, class_labels=labels
                )
                if guidance_scale > 0:
                    uncond_output = diffusion_model(
                        image, timesteps=timesteps, context=uncond_conditioning, class_labels=uncond_labels, clinical_data = clinic
                    )
                    model_output = torch.lerp(uncond_output, model_output, guidance_scale)
...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
        ...
        """
        with torch.no_grad():
            latent = autoencoder_model(inputs, 'encode_2_inputs') * self.scale_factor
    ...
    def sample(
        ...
        image = autoencoder_model(latent / self.scale_factor, 'decode')

        if save_intermediates:
            intermediates = []
            for latent_intermediate in latent_intermediates:
                intermediates.append(autoencoder_model(latent_intermediate / self.scale_factor, 'decode'))
            return image, intermediates
