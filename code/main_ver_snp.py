...
class Training(object):
    ...
    def autoencoder(self):
        ...
        optimizer_g = torch.optim.Adam(self.autoencoderkl.parameters(), 
                                       lr=self.config.train.g_lr,) 
        ...
        for epoch in range(n_epochs):
            ...
            gen_epoch_loss = 0
            disc_epoch_loss = 0
            progress_bar = tqdm(enumerate(self.ae_train_loader), total=len(self.ae_train_loader), ncols=100)
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                path = os.path.join("results", self.config.run_name, f"AE_{epoch}.png")
                save_images(reconstruction[0], path)
        progress_bar.close()
    ...
    def diffusion(self):
        ...
        for epoch in range(n_epochs):
            ...
            for step, batch in progress_bar:
                ...
                with autocast(enabled=True):
                    ...
                    if self.config.diffusion.mode == 'concat':
                        snps = snps.unsqueeze(-1).unsqueeze(-1).expand(-1, -1, noise.shape[-2], noise.shape[-1])
            ...
            if (epoch + 1) % val_interval == 0:
                ...
                # if labels[0].item() == 0:
                #     save_images(decoded[0], os.path.join(path, "AD", f"{epoch}.jpg"))
                # elif labels[0].item() == 1:
                #     save_images(decoded[0], os.path.join(path, "CN", f"{epoch}.jpg"))
                # elif labels[0].item() == 2:
                #     save_images(decoded[0], os.path.join(path, "MCI", f"{epoch}.jpg"))
        ...
        unet.eval()
        batch = next(iter(self.dm_test_loader))
        ...
        for i in range(n):
            if labels[i].item() == 0:
                save_images(decoded[i], os.path.join(path, "AD", f"{i}.jpg"))
            elif labels[i].item() == 1:
                save_images(decoded[i], os.path.join(path, "CN", f"{i}.jpg"))
            elif labels[i].item() == 2:
                save_images(decoded[i], os.path.join(path, "MCI", f"{i}.jpg"))
        
        # for i in range(n):
        #     save_images(decoded[i], os.path.join(path, f"sample_{step}_{labels[i].item()}.jpg"))
        # decoded = decoded.clamp(0, 1)
        #     # save_images(decoded, os.path.join(path, f"{epoch}.jpg"))
...
if __name__ == '__main__':
    ...
    if args.type == 'train':
        ...
        with wandb.init(project= "MRI_ldm", group= "SNP_LABEL",  config= config) if config.train.use_wandb else nullcontext():
            if args.stage == 'both':
