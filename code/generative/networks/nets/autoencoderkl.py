...
class AutoencoderKL(nn.Module):
    ...
    def __init__(
        ...
        if isinstance(num_res_blocks, int):
            num_res_blocks = ensure_tuple_rep(num_res_blocks, len(num_channels)) # ensure_tuple_rep(1, 3) -> (1, 1, 1)
    ...
    def forward(self, x: torch.Tensor, stage: str=None): # -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        if stage == 'encode':
        ...
        #     return z
            ...
            z_mu, z_sigma = self.encode(x) #(bs, 3, 64, 64)
            z = self.sampling(z_mu, z_sigma)
