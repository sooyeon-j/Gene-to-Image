...
class CrossAttention(nn.Module):
    ...
    def reshape_heads_to_batch_dim(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        batch_size, seq_len, dim = x.shape #when x is context, (bs, 1, 256)
        x = x.reshape(batch_size, seq_len, self.num_heads, dim // self.num_heads) #x: (bs, 256, 1, 256), context: (bs, 1, 1, 256)
        x = x.permute(0, 2, 1, 3).reshape(batch_size * self.num_heads, seq_len, dim // self.num_heads) #x: (bs, 256, 256), context: (bs, 1, 256)
        return x
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, context: torch.Tensor | None = None) -> torch.Tensor:
        ...
        if self.spatial_dims == 2:
            x = x.permute(0, 2, 3, 1).reshape(batch, height * width, inner_dim) #(bs, 256, 256)
        if self.spatial_dims == 3:
...
def get_timestep_embedding(timesteps: torch.Tensor, embedding_dim: int, max_period: int = 10000) -> torch.Tensor:
    ...
    embedding = torch.cat([torch.cos(args), torch.sin(args)], dim=-1) #(bs, 128)
...
#n_embed: 512?, vocab_size: snp variables (0,1,2), max_seq_len: snp size
...
class DownBlock(nn.Module):
    ...
    # hidden_states: (bs, 128, 32, 32), same as h after the initial convolution
    def forward(
        ...
        return hidden_states, output_states # hidden_states: (bs, 128, 16, 16)
        ...
        for i in range(num_res_blocks):
            ...
            attentions.append(
                SpatialTransformer(
                    ...
                    num_attention_heads=out_channels // num_head_channels, # 1
                    num_head_channels=num_head_channels,
...
class VectorQuantizer(nn.Module):
    def __init__(self, n_e, e_dim, beta):
        ...
        self.n_e = n_e  # number of embedding
        self.e_dim = e_dim  # embedding dimension
        self.beta = beta  # weight for the reconstruction loss
...
class DiffusionModelUNet(nn.Module):
    ...
    def __init__(
        ...
        transformer_num_layers: int = 1,
        cross_attention_dim: int | None = None,
        num_context_embeds: int | None  = None,
        num_class_embeds: int | None = None,
        num_clininal_embeds: int | None = None,
        upcast_attention: bool = False,
        use_flash_attention: bool = False,
    ) -> None:
        super().__init__()
        ...
        if isinstance(num_res_blocks, int):
            num_res_blocks = ensure_tuple_rep(num_res_blocks, len(num_channels)) #ensure_tuple_rep('test', 3) -> ('test', 'test', 'test')
        ...
        if num_clininal_embeds is not None:
            self.clinical_embedding = nn.Linear(num_clininal_embeds, time_embed_dim)

        # snp embedding
        if cross_attention_dim is not None and with_conditioning is True:
        #     self.context_embedding = VectorQuantizer(n_e=1024, e_dim=50, beta=0)
            
            self.snp_embedding = TransformerEmbedder(n_embed=cross_attention_dim, n_layer=32, vocab_size=num_class_embeds, max_seq_len=num_context_embeds)
        
        # self.num_context_embeds = num_context_embeds
        # if (cross_attention_dim is not None) and (num_context_embeds is not None):
        #     self.context_embedding = nn.Embedding(num_context_embeds, cross_attention_dim)
        #     self.position_embedding = nn.Embedding(num_context_embeds, cross_attention_dim)
    ...
    def forward(
        ...
        clinical_data: torch.Tensor | None = None,
        context: torch.Tensor | None = None,
        ...
        emb = self.time_embed(t_emb) # (bs, 512)
        ...
        if clinical_data is not None:
            clinical_emb = self.clinical_embedding(clinical_data)
            clinical_emb = clinical_emb.to(dtype=x.dtype)
            emb = emb + clinical_emb

        if context is not None:
        #     context_emb = self.context_embedding(context).unsqueeze(1)
        #     context = context_emb
            context = self.snp_embedding(context) # (bs, 50, 512)

        #     pos_emb = self.position_embedding(context)
        #     context = context_emb + pos_emb

        # 3. initial convolution
        h = self.conv_in(x) # (bs, 128, 32, 32)
